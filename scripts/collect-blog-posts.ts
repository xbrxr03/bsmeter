...
async function main() {
  const seedFile = process.argv[2] ?? "data/seeds/blog-posts.json";
  const seeds = JSON.parse(await readFile(seedFile, "utf8")) as Seed[];
  const out = join(process.cwd(), "data", "bake-off", "content-seo");
  ...
  console.log(`Wrote ${count} blog samples from ${seedFile}.`);
}
